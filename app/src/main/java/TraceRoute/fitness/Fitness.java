...
public class Fitness {
    ...
    public static Boolean intersect(Point2D p1, Point2D p2, Point2D p3, Point2D p4)
    {
        try {
            ...
            if(between(intersection.getX(),p1.getX(),p2.getX())&&between(intersection.getX(),p3.getX(),p4.getX())&&between(intersection.getY(),p1.getY(),p2.getY())&between(intersection.getY(),p3.getY(),p4.getY()))
            {
                return true;
            }
            else
    ...
    public static Double Perpendicular(RTree<String, Geometry> london, PathIterator pathiterator, double dx, double dy, double scale, double searchdist) {
        ...
        for (Point2D coordinate : pointslist
            ...
            Line searchline = Geometries.line(lon1,lat1,lon2,lat2);
            Iterable<Entry<String,Geometry>> online = london.search(searchline).toBlocking().toIterable();
